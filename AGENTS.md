...
5. **Interactive follow-up loops reset turn count.** Every user follow-up prompt must reset the current execution loop `turn` counter to `0`, ensuring a fresh 15-turn budget for the follow-up prompt.
6. **Turn-wide Ctrl+C/SIGINT handlers must verify prompt activity.** When intercepting interrupts during turns, the listener must check `isPromptActive()`. If an interactive prompt is active, it must return early and let the prompt components handle the escape locally (denying the tool call) rather than triggering the turn-wide abort.
7. **Wrap code previews *before* applying syntax highlighting.** To prevent slicing through TrueColor ANSI styling sequences, plain text code lines must be wrapped to the TUI column widths in raw formats first, and then each slice highlighted separately.
8. **Message history pruning must always remove `assistant`+`tool_calls` and their matching `tool` results as atomic pairs.** Never prune one without the other — this breaks OpenAI-compatible message format validation on the next request.
9. **`${...}` template variables in `sys.txt` substitution must always use global regex replacement, never a plain string `.replace()`** — a non-global replace only fills the first occurrence and leaks a literal `${...}` token into the system prompt on any repeat use.
