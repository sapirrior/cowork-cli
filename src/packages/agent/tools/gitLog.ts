 ...
 * @param args
 * @param [args.limit=10]       Number of commits to retrieve (max: 50).
 * @param [args.oneline=false]  Compact single-line format per commit.
 */
...
export default async function gitLog({ limit = DEFAULT_LIMIT, oneline = false }: GitLogArgs): Promise<string> {
  // Clamp limit
