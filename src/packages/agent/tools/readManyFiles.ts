 ...
 * @param args.include Glob patterns of files to read.
 * @param [args.exclude=[]] Glob patterns of files to ignore.
 * @param [args.useDefaultExcludes=true] Whether to apply default ignores.
 * @param [args.respectGitIgnore=true] Whether to apply .gitignore patterns.
 */
...
export default async function readManyFiles({
...
}: ReadManyFilesArgs): Promise<string> {
  ...
  let rootIgnoreList: IgnorePattern[] = [];
  if (respectGitIgnore || useDefaultExcludes) {
    rootIgnoreList = await getIgnorePatterns();
  }

  const walk = async (currentPath: string, depth = 0, ignoreList: IgnorePattern[] = rootIgnoreList): Promise<void> => {
