...
| :--- | :--- | :--- | :--- |
| `index.js` | All helpers | Export | Bundles and exposes utility modules. |
| `helpMsg.js` | `show_help` | Function | Prints usage guidance, parameter flags, and configuration examples to the console when `cwk -h` or `cwk --help` is requested. |
| `logger.js` | `logger` | Object | Exposes print wrappers (`main`, `secondary`, `error`). |
...
| `fsUtils.js` | `getIgnorePatterns` | Function | Loads default ignore rules plus root `.gitignore` pattern strings. Caches outputs. |
...
### `fsUtils.js` Implementation Rules
