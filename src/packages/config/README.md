...
| `configManager.ts` | `loadConfig()` | Function | Reads credentials, performing schema validation and logging parsing/validation errors rather than silently swallowing them. Prioritizes the JSON structure in `~/.config/haiku/auth.json` (modern multi-provider configuration). Falls back to legacy `.env` in the user's home folder. |
...
| | `loadAuthFile()` | Function | Reads `~/.config/haiku/auth.json` into memory and checks for formatting errors. |
| | `saveAuthFile(authConfig)` | Function | Writes the updated multi-provider JSON file atomically (via a temporary file swap) with secure permissions (mode 0o600) to `~/.config/haiku/auth.json`. |
