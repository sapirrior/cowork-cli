 ...
 */
export async function selectProvider(uiInstance, items) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const onData = (chunk) => {
      ...
      if (str === '\u0003') { // Ctrl+C
        ...
        const dimColor = [194, 198, 197];
        
        process.stdout.write(`${rgb(blueColor, '◇')} ${rgb(dimColor, 'cwk login')}\n`);
 ...
 */
export async function inputField(uiInstance, { label, hint, masked = false, required = false, fallback = '' }) {
  uiInstance._abortActiveSpinner();
  if (!process.stdin.isTTY) {
 ...
 */
export async function runFieldSequence(uiInstance, fields) {
  const results = {};
